...
@Directive()
export abstract class AbstractEntityFormComponent<E extends object>
  implements OnInit
{
  ...
  public ngOnInit(): void {
    ...
    this.subscribe(
      this.errorService.globalErrors$,
          ...
          ([fieldName, errorMessage]): void => {
            ...
            if (this.entityForm.contains(transformedFieldName)) {
              const formFieldControl =
                this.entityForm.get(transformedFieldName);
              if (formFieldControl) {
