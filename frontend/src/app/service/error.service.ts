...
export class ErrorService {
  ...
  public handleHttpErrorResponse(httpErrorResponse: HttpErrorResponse): void {
    if (httpErrorResponse.error instanceof ErrorEvent) {
      ...
      let error: ErrorResponse = this.buildError("CLIENT", "Unknown error");
      this.push(error);
    } else {
      switch (httpErrorResponse.status) {
        case 0:
          let error: ErrorResponse = this.buildError("NETWORK", "Connection error");
          this.push(error);
          break;
        case 422:
        ...
        default:
          try {
            let error: ErrorResponse = httpErrorResponse.error as ErrorResponse;
            this.push(error)
          } catch (e) {
            console.log(e);
            let error: ErrorResponse = this.buildError("EXCEPTION", "Unknown error");
            this.push(error);
          }
      }
    }
  ...
  private handleFieldErrors(httpErrorResponse: HttpErrorResponse): void {
    ...
    for (let fieldError of error.entityFieldValidationErrors) {
